...
router
  .delete('accounts/:id', [AccountsController, 'destroy'])
...
router
  ...
  .use(middleware.auth({ guards: ['api'] }))
router
  .get('transactions/search', [TransactionsController, 'search'])
  .use(middleware.auth({ guards: ['api'] }))
router
  .get('transactions/summary/day', [TransactionsController, 'summaryByDay'])
  .use(middleware.auth({ guards: ['api'] }))
router
  .get('transactions/summary/category', [TransactionsController, 'summaryByCategory'])
  .use(middleware.auth({ guards: ['api'] }))
router
  .get('transactions/summary/account', [TransactionsController, 'summaryByAccount'])
  .use(middleware.auth({ guards: ['api'] }))
router
  .get('transactions/summary/week', [TransactionsController, 'summaryByWeek'])
  .use(middleware.auth({ guards: ['api'] }))
router
  .get('transactions/:id', [TransactionsController, 'show'])
  .use(middleware.auth({ guards: ['api'] }))
router
  .post('transactions', [TransactionsController, 'store'])
  .use(middleware.auth({ guards: ['api'] }))
router
